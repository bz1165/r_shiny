...
inject_compat_shims <- function(target_env) {
  ...
  # If you want `filter` available, expose dplyr::filter (only if dplyr is installed)
  if (requireNamespace("dplyr", quietly = TRUE)) {
    assign("filter", getExportedValue("dplyr", "filter"), envir = target_env)
  }
  ...
  if (requireNamespace("haven", quietly = TRUE)) {
    assign("read_sas", getExportedValue("haven", "read_sas"), envir = target_env)
    assign("read_xpt", getExportedValue("haven", "read_xpt"), envir = target_env)
     assign("write_xpt", getExportedValue("haven", "write_xpt"), envir = target_env)
     assign("as_factor", getExportedValue("haven", "as_factor"), envir = target_env)
  } else {
    # Fail fast with a clear message inside the sandbox
    assign("read_sas", function(...) {
      stop("Package 'haven' is required for read_sas(); please ensure it is available in the training sandbox.")
    }, envir = target_env)
  }
  
  
  if (requireNamespace("rtables", quietly = TRUE)) {
    for (fn in c("basic_table","split_cols_by","add_overall_col","split_rows_by",
                 "summarize_row_groups","analyze","build_table",
                 "in_rows","rcell")) {
      assign(fn, getExportedValue("rtables", fn), envir = target_env)
    }
  }
...
make_training_wd <- function() {
  file.path(CONFIG$TRAINING_RA_ROOT, "pgm")
}
...
load_setup_env <- function(setup_timeout_sec = 30, force_reload = FALSE) {
  ...
  setup_res <- tryCatch({
    ...
    # --- NEW: preload general funcs so that set_paths/read_df helpers exist
    if (!is.null(funcs_general_path)) {
      sys.source(funcs_general_path, envir = setup_env)
    }
    
    # Optional: assert set_paths is available before sourcing _setup.R
    if (!exists("set_paths", envir = setup_env, inherits = FALSE)) {
      stop(paste(
        "set_paths() not loaded from funcs_general.R.",
        "Checked at: ", funcs_general_path %||% "<none>",
        "Ensure util/funcs_general.R defines a top-level set_paths().",
        sep = "\n - "
      ))
    }
